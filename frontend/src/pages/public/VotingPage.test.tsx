...
// VotingPage goi them /teams song song voi /tallies; thieu mock nay thi
// Promise.all bi reject va khong the doi nao duoc render.
...
function setupApiMocks() {
  mockedApi.get.mockImplementation((url: string) => {
    ...
    if (url === `/api/public/voting/tracks/${TRACK.id}/teams`) return Promise.resolve({ data: [TEAM] });
...
describe("VotingPage castVote", () => {
  ...
  it("records the vote in localStorage on success", async () => {
    ...
    const voteButton = await screen.findByText("Bình chọn cho đội này");
    await userEvent.click(voteButton);

    await waitFor(() => expect(localStorage.getItem(votedKey(TRACK.id))).toBe(TALLY.teamId));
    expect(await screen.findByText("Bạn đã bình chọn cho đội này")).toBeInTheDocument();
  });
  ...
  it("does NOT lock the visitor out when the vote request fails (regression test)", async () => {
    ...
    expect(screen.getByText("Bình chọn cho đội này")).toBeEnabled();
  });
});
