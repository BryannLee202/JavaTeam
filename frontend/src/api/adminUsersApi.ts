...
export const adminUsersApi = {
  listPending: (page = 0, size = 200) =>
    api.get<Page<UserSummary>>("/api/admin/users/pending", { params: { page, size } }).then((res) => res.data),
    
  listApproved: (page = 0, size = 200) =>
    api.get<Page<UserSummary>>("/api/admin/users/approved", { params: { page, size } }).then((res) => res.data),
